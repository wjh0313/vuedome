...
<script>
export default {
	name:'teaCourseManger',
	data() {
		return {
			isActives: 0,
			arrt1: ["我的课程",'我的活动'],
			arrt2: ['icon-rili','icon-jingbao'],
			arrlink:['/person/courseManger/myCourse','/person/courseManger/myActive'],
		};
	},
	methods: {
	...
	mounted() {
		...
		if (this.$route.path.split("/")[3] == "myCourse") {
			this.isActives = 0;
		} else if (this.$route.path.split("/")[3] == "myActive") {
			this.isActives = 1;
		} 
	},
	watch: {
